...
-- TYPE 제작
CREATE OR REPLACE TYPE 거래종류 AS OBJECT
("평균거래" NUMBER, "최소거래" NUMBER, "최대거래" NUMBER);
/

SELECT 고객번호, 고객명, A.T."평균거래", A.T."최소거래", A.T."최대거래"
FROM (SELECT 고객번호, 고객명,
             (
                 SELECT 거래종류(AVG(거래금액), MIN(거래금액), MAX(거래금액))
